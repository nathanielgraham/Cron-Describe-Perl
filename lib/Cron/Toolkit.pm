...
=head1 SYNOPSIS
  ...
  # Crontab string (supports @aliases)
...
Parses a crontab string into array of Cron::Toolkit objects. Skips comments (#), empty lines, env vars (e.g., SHELL=), invalid exprs (warns). Extracts user/command if present (system crontabs). Supports @aliases.
Parameters:
=over 4
=item *
input: Multi-line crontab content as string.
=back
Returns: Array of valid objects (empty if none). Objects include C<user> and C<command> attrs if extracted.
=head2 describe
...
sub _new {
   ...
   if ( $args{is_quartz} ) {
      ...
      push( @fields, '*' ); # year
      # Pre-collapse degenerate DOW steps (Unix 0-7)
      if ( $fields[5] =~ /^(\d*|\*)\/(\d+)$/ ) {
...
sub new_from_crontab {
   ...
   foreach my $line ( split /\n/, $content ) {
      ...
      # Skip env var lines (KEY=VALUE)
      next if $line =~ /^[A-Z_][A-Z0-9_]*=/;
