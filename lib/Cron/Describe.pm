...
Cron::Describe - Parse and describe cron expressions (Unix + Quartz)
...
=head1 SYNOPSIS
  ...
  # Unix (auto-detected)
  my $cron = Cron::Describe->new(expression => "0 0 * * 1");

  # Quartz
  my $cron = Cron::Describe->new_from_quartz(expression => "0 0 0 15 * ? *");
  ...
  say $cron->is_valid;     # 1
  say $cron->is_match($tm); # 1 or 0

=head1 DESCRIPTION

B<Cron::Describe> parses Unix and Quartz cron expressions with automatic format detection.
...
=head3 C<new>
  ...
  my $cron = Cron::Describe->new(
      expression => "0 0 * * 1",  # Auto-detects Unix
      tz => "America/New_York"    # Optional
  );

Auto-detects format and calls the appropriate parser.

=head3 C<new_from_unix>

  my $cron = Cron::Describe->new_from_unix(expression => "0 0 * * 1");

Explicit Unix format (5 fields).

=head3 C<new_from_quartz>

  my $cron = Cron::Describe->new_from_quartz(expression => "0 0 0 15 * ? *");

Explicit Quartz format (6-7 fields).

=head2 METHODS

=head3 C<is_valid>

  say $cron->is_valid;  # 1 or 0

=head3 C<error_message>

  say $cron->error_message;  # "Invalid field" or ""
...
=head2 Unix (5 fields, auto-detected)

  MINUTE HOUR DOM MONTH DOW
  0      0    *    *    1    # Monday 00:00

=head2 Quartz (6-7 fields)

  SEC  MIN  HOUR DOM  MONTH DOW  YEAR
  0    0    0    15   *     ?    *     # 15th 00:00:00
...
=head1 TIMEZONE
  ...
  tz => "America/New_York"  # -300 minutes
  tz => "Asia/Tokyo"        # +540 minutes
...
Copyright 2025 Your Name. MIT License.
