...
our %templates = (
   ...
   every_N_hour => 'every {step} hours starting at {start}',
...
sub normalize {
    ...
    @fields = map { name_to_num($_) || $_ } @fields;
...
sub validate {
    ...
    die "Syntax: Invalid chars in $field_type: $expr" if $expr =~ /[^0-9*,\/\-L#W?]/;
    ...
    if ($expr =~ /^(\d+)(?:[\/\-#W])?(\d*)$/) {
        ...
        die "$field_type nth $val2 invalid (max 5)" if $field_type eq 'dow' && $val2 > 5;
    }
