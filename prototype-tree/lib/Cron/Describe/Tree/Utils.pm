...
our @EXPORT_OK = qw(
  ...
  ordinal_list step_ordinal complex_join normalize validate generate_list_desc %limits
...
our %limits = (
   ...
   dom => [ 1, 31 ], month => [ 1, 12 ], dow => [ 1, 7 ], year => [ 1970, 2099 ]
);
...
sub normalize {
   ...
   while ( my ( $name, $num ) = each %dow_map ) { $expr =~ s/\b\Q$name\E\b/$num/gi; }
...
sub validate {
   ...
   # 🔥 SIMPLE: Just reject malformed L/LW
