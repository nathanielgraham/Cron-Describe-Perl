...
sub parse_field {
   ...
   if ( $field eq '*' ) {
      ...
      return Cron::Describe::Tree::LeafPattern->new( type => 'last', value => 'L' );
   }
   elsif ( $field =~ /^L-(\d+)$/ ) {
      return Cron::Describe::Tree::LeafPattern->new( type => 'last', value => $field );  # 🔥 FIXED: $field not 'L-$1'
   }
   elsif ( $field =~ /^LW$/ ) {
      return Cron::Describe::Tree::LeafPattern->new( type => 'lastW', value => 'LW' );
   }
   elsif ( $field =~ /^\d+$/ ) {
